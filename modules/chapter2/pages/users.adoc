...
The terms `Namespaces`, OpenShift `Projects`, and `Data Science Projects` are often used synonymousy.  While there are technical differences between these objects, a project is essentially a namespace with some additional features provided by OpenShift.  OpenShift will always create a corresponding project for every namespace, and vice versa.  
